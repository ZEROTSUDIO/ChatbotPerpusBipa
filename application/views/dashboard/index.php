...
class ChatStatsDashboard {
    ...
    async loadInitialData() {
        try {
            // Load overview stats
            const overviewResponse = await fetch('/ChatbotPerpusBipa/api/stats/overview');
            const overviewData = await overviewResponse.json();
            this.updateOverviewCards(overviewData.data);

            // Load chart data
            const chartsResponse = await fetch('/ChatbotPerpusBipa/api/stats/charts');
            const chartsData = await chartsResponse.json();
            this.updateCharts(chartsData.data);

            // Load intent performance
            const intentResponse = await fetch('/ChatbotPerpusBipa/api/stats/intent-performance');
            const intentData = await intentResponse.json();
            this.updateIntentTable(intentData.data);

            // Load activity feed
            const activityResponse = await fetch('/ChatbotPerpusBipa/api/stats/recent-activity');
            const activityData = await activityResponse.json();
            this.updateActivityFeed(activityData.data);
    ...
    setupEventListeners() {
        ...
        document.getElementById('volumePeriod').addEventListener('change', async (e) => {
            ...
            const response = await fetch(`/ChatbotPerpusBipa/api/stats/volume?period=${period}`);
